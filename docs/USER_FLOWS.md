...
1. Пользователь выбирает dynamometer или gripper board и фильтры.
2. Отправляет результат, дату и URL видео с разрешённого хоста.
3. Сервер принудительно создаёт `pending`; пользователь не может self-approve.
4. Admin approve/reject через защищённый RPC.
5. Автор получает realtime/local/remote verdict notification и видит статус заявки.
