...
- Роли доступа — таблица `user_roles` (`grip` — дефолт новорегов, комьюнити-режим без ИИ; `full` — вся апка; `admin` — full + модерация лидерборда). Каждая ИИ-функция проверяет роль на сервере через `_shared/roles.ts` — скрытые табы защитой не считаются. Роль **никогда** не переносить в `profile` (юзер может править свою строку профиля).
