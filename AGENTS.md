...
- Роли доступа — таблица `user_roles`. `grip` — дефолт всех новорегов: свободная тренировка без программ, эспандеры/grip-рекорды, аналитика и лидерборд; без ИИ/OURA/цикла. `full` — private-режим только Марии; `admin` — private-режим Сергея + модерация лидерборда. Не выдавать `full/admin` другим пользователям без явного решения владельца. Каждая ИИ-функция проверяет роль на сервере через `_shared/roles.ts` — скрытые табы защитой не считаются. Роль **никогда** не переносить в `profile` (юзер может править свою строку профиля).
...
- `npm test -- --runInBand` — Jest unit/component tests; не заменяет device/E2E smoke.
